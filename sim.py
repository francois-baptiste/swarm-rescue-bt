...
grid for victims and rescue them. There is no central coordinator: task
allocation emerges from robots broadcasting claims over a simulated radio
and each robot independently applying the same rule to what it hears.
SCENARIOS below covers several maps/robot-victim ratios/failure patterns,
all using the exact same allocation rule, so their outcomes can be
compared against the centralized branch's own SCENARIOS (kept parameter-
for-parameter identical) to see which architecture suits which situation.
...
# branch's sim.py (same map, victims, starts, sensor_range, failures,
# max_ticks) - that's what makes `--compare`'s ticks-to-complete numbers
# meaningful to compare *across* branches, not just within one.
SCENARIOS = {
    "default": dict(
...
def render(world, robots, frames, gif_path=None, live=False, slider=False):
    ...
    def draw(i):
        ...
        ax_map.set_title(f"Decentralized SAR swarm ({SCENARIO}) - tick {f['t']}")
...
def compare(names):
    ...
    all_ok = True
    for name in names:
        world, bus, robots, spec = build_scenario(name)
        frames = run(world, bus, robots, spec)
...
if __name__ == "__main__":
    ...
    world, bus, robots, spec = build_scenario(SCENARIO)
