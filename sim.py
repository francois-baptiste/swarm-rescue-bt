"""Decentralized multi-robot swarm demos, driven by behavior trees.

Every mission below is solved with no central coordinator: whatever
coordination happens emerges from robots broadcasting over a simulated
radio and each robot independently applying the same local rule to what
it hears - compare this to the `centralized-swarm` branch, where a single
Coordinator makes every allocation decision.
...
Missions (see missions/<name>.py for each one's own docstring):
  ...
  patrol        - walk a shared border loop, split into per-robot arcs
  relay         - hold a chain of positions to relay a message source -> sink
